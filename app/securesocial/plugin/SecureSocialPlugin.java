...
public class SecureSocialPlugin extends PlayPlugin {
	...
	private static final String SECURESOCIAL_SERVICE = "securesocial.service";

	@Override
	public void onApplicationStart() {
		// register providers
		final List<Class> providers = Play.classloader
				.getAssignableClasses(IdentityProvider.class);
		if (providers.size() > 0) {
			...
			final String s = Play.configuration
					.getProperty(SECURESOCIAL_PROVIDERS);

			if (s != null && s.length() > 0) {
				final String[] requestedProviders = s.split(SEPARATOR);
				for (String type : requestedProviders) {
					try {
						ProviderRegistry.register(availableProviders
								.get(ProviderType.valueOf(type)));
					} catch (IllegalArgumentException e) {
						Logger.error(
								"Unknown type specified in securesocial.providers: %s",
								type);
					}
				}
			} else {
		...
		final List<Class> classes = Play.classloader
				.getAssignableClasses(UserService.Service.class);
		UserService.Service service = null;
		service = getCustomUserService();
		if (service == null) {
			...
			if (classesFound == 1) {
				...
				// a custom implementation was found. use it instead of the
				// default
				...
				// should not happen unless someone implements the interface
				// more than once.
	...
	private Service getCustomUserService() {
		String customUserService = Play.configuration
				.getProperty(SECURESOCIAL_SERVICE);
		UserService.Service service = null;
		if (customUserService != null) {
			try {
				Class c = Class.forName(customUserService);
				service = (Service) newInstance(c);
				Logger.info("Using custom user service: %s", service.getClass());
			} catch (ClassNotFoundException e) {
				throw new RuntimeException(
						"Unable to create Custom User Service "
								+ customUserService, e);
			}
		}
		return service;
